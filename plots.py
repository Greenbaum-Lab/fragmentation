...
from funcs2 import frag_rand, frag_cor, frag_dist, het_rand, het_cor, het_dist, make_networks, make_iterations, \
...
nets = make_networks(n_nets=50, n_nodes=n, connectivity=p, net_type='RGG')
...
breaking_point_rand = find_breakink_point_list(rand[2])
breaking_point_cor = find_breakink_point_list(cor[2])
breaking_point_dist = find_breakink_point_list(dist[2])
...
plt.axvline(x=mean(breaking_point_rand), color=color_palette(0),ymax=0.1)
plt.axvline(x=mean(breaking_point_cor), color=color_palette(1),ymax=0.1)
plt.axvline(x=mean(breaking_point_dist), color=color_palette(2),ymax=0.1)

# Add labels and legend
plt.xlabel('Fragmentation step')
plt.ylabel('Pairwise Fst')
plt.title('50 nodes, 50 reps, 0.4, includeing ones ')
plt.legend()

# Display the plot
plt.show()
plt.savefig("fst.svg", format="svg")
